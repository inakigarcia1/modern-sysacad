...
- [x] **Capa de caché híbrida** — `MS_CACHE` sobre `sessionStorage` (data ya parseada por página, versionada + TTL) + `fetchDoc` con dedupe + prefetch en hover de links internos. Los getters (`getPlanRows`, `getEstadoMap`, `getPlanTotal`) hacen cache → fetch → extract. Estado y Plan alimentan la caché al visitarlos.
